...
def getWebPage():
    ...
    if time.time() > last_web_update+10:  # update at every 10 seconds at most
        ...
        #page = urllib.urlopen("http://www.weatherlink.com/user/oceanfrontcondos/") # Gulotta Communications, Newton, MA
...
def getWindSpeed():
    ...
    speedMatchObject = re.search(windSpeedPattern, pageText)
    if (speedMatchObject == None):
        windSpeed = 0
    else:
        windSpeed = float(speedMatchObject.group(1))
    ...
    if (windSpeed<1):
        windSpeed=1 # prevent the speed with random variation from making windSpeed below 1mph
